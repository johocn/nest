...
| # | 项 | 证据 | 影响 |
|---|---|---|---|
| P0-1 | 成就进度不推进 | event-listeners.service.ts:116,124 | 成就永久停在 0 |
| P0-2 | 成就领奖不发奖 | achievement.service.ts:97-105 | 发奖丢失，状态不可回滚 |
| P0-3 | 排行 score 恒 0 | ranking.service.ts:46-54,74-87 | 榜单与快照数据均无分值 |
...
1. **已上线的功能里有 8 处"看起来能用、实际不对"**（P0），其中成就与排行属对玩家可见的功能性缺陷。
