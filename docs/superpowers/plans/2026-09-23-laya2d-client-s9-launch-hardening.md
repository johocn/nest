...
- **⚠️ T3 已踩到门槛线（本轮最值得盯的数）**：§7.4.3 门槛档要求 **T3 ≤ 5s**，而生产实测（冷缓存首访）**T3 = 6.35 / 7.33 / 6.76s，三档全部 > 5s**；拆开看 **T1 ≈ 2.9–3.4s（页面到登录表单可输入）、T2 ≈ 3.5–3.9s（点登录到场景首帧）**，两段各占一半。这是移动模拟口径（本机出网的公网往返，非真机 4G/WiFi），**不能直接判 A4 FAIL**，但它说明「≤5s」这条**没有余量**：真机若也 >5s 即门槛档 FAIL。**建议先查 T1**（首访要拉 index.html + 60 js + 5 libs + 28 assets，未压缩/未合并 + 无 CDN，是最大嫌疑）。**真机点检时务必单独记录 T1 与 T2**，以便区分「前端装载慢」还是「登录+进场景慢」。
